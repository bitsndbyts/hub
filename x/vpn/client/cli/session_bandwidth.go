...
func UpdateFreeSessionInfoTxCmd(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-free-session-info",
		Short: "Update free session info",
		RunE: func(cmd *cobra.Command, args []string) error {
			txb := auth.NewTxBuilderFromCLI().WithTxEncoder(utils.GetTxEncoder(cdc))
			ctx := context.NewCLIContext().WithCodec(cdc)
			...
			_nodeID, err := hub.NewNodeIDFromString(nodeID)
			if err != nil {
				return err
			}

			msg := types.NewMsgUpdateFreeSessionBandwidth(ctx.FromAddress, _nodeID, clientID, bandwidth)

			return utils.GenerateOrBroadcastMsgs(ctx, txb, []sdk.Msg{msg})
		},
	}

	cmd.Flags().String(flagClientID, "", "ClientID")
	cmd.Flags().String(flagNodeID, "", "NodeID")
	cmd.Flags().Int64(flagUpload, 0, "Upload in in bytes")
	cmd.Flags().Int64(flagDownload, 0, "Download in bytes")
