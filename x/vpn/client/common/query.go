...
func QueryFreeSessionsOfClient(ctx context.CLIContext, id string) ([]types.FreeSession, error) {
	params := types.NewQueryFreeSessionsOfClientParams(id)
	...
	path := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryFreeSessionsOfClient)
	res, _, err := ctx.QueryWithData(path, bytes)
	if err != nil {
		return nil, err
	}
	...
	var sessions []types.FreeSession
	if err := ctx.Codec.UnmarshalJSON(res, &sessions); err != nil {
		return nil, err
	}
